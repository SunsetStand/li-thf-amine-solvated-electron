...
log paths. Use `./run.sh inspect` for the latest short status/log summary, or
`./run.sh inspect JOBID` for a specific controller job. `./run.sh queue` is only
a live scheduler query and creates no log.
